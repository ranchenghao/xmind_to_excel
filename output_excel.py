...
def write_excel(data):
    ...
    for case in get_value_from_json(data[0]['topic'], [], []):
        ...
        if case.get('topics'):
            ws['D{}'.format(r)] = case.get('topics')[0]['title']
        if case['topics'][0].get('topics'):
            ws['E{}'.format(r)] = case['topics'][0]['topics'][0]['title']
        if case['topics'][0]['topics'][0].get('topics'):
            ws['F{}'.format(r)] = case['topics'][0]['topics'][0]['topics'][0]['title']
        if case['topics'][0]['topics'][0]['topics'][0].get('makers'):
            ws['I{}'.format(r)] = '高'
